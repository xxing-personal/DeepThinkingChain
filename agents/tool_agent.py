...
class ToolAgent(Agent):
    ...
    def get_tools_prompt(self) -> str:
        ...
        Returns:
            String prompt describing available tools
        """
        if len(self.tools) == 1:
            tool_name = list(self.tools.keys())[0]
            tool = self.tools[tool_name]
        ...
        tool_categories = getattr(self, 'tool_categories', {})
        if tool_categories:
            for category in sorted(tool_categories.keys()):
                prompt += f"Category: {category}\n"
                
                for tool_name in sorted(tool_categories[category]):
            ...
            # Just list tools without categories
